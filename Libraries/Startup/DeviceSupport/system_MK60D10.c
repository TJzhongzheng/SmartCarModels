...
void SystemInit (void) {
  ...
  MCG->C2 = (uint8_t)0x24u;
#elif (CLOCK_SETUP == 3)
    SIM->CLKDIV1 = (uint32_t)0xFFFFFFFFu;      /* Configuration of System Predivider. Pre-set to minimum frequency division*/
    OSC->CR = (uint8_t)0x00u;
    ...
    while((MCG->S & MCG_S_PLLST_MASK) == 0u);           /* Waiting for the PLLS clock source to go to PLL */
    while((MCG->S & MCG_S_LOCK0_MASK) == 0u);           /* waiting for lock */
    /* enable PLL */
    MCG->C1 = (uint8_t)0x1Au;
    while((MCG->S & 0x0Cu) != 0x0Cu);                   /* Waiting for PLL output */
    while((MCG->S & MCG_S_LOCK0_MASK) == 0u);           /* Waiting for PLL lock */
#elif (CLOCK_SETUP == 4)
	//SIM->CLKDIV1 = (u32)0x00110000u; //Configuration of System Predivider
	SIM->CLKDIV1 = (uint32_t)0xFFFFFFFFu;
	// transform to FEI mode 
	MCG->C1 = (uint8_t)0x06u;
	MCG->C2 = (uint8_t)0x00u;
	MCG->C4|= (1<<6)|(1<<7)|(1<<5);   //Internal slow reference clock is 32.768KHZ. Frequency doubling factor 2197 is 96MHZ. Refer to MCG->C4 register.
	//Frequency division strategy:
	//SIM_CLKDIV1_OUTDIV1(0) CORE     CLOCK  1 Frequency Division 	UP TO 100M  
	//SIM_CLKDIV1_OUTDIV2(1) BUS      CLOCK  2 Frequency Division		UP TO 50M 
	//SIM_CLKDIV1_OUTDIV3(1) FlexBus  ClOCK  2 Frequency Division   UP TO 50M 
	//SIM_CLKDIV1_OUTDIV4(3) Flash    ClOCK  3 Frequency Division   UP TO 25M 
	SIM->CLKDIV1 =(SIM_CLKDIV1_OUTDIV1(0)|SIM_CLKDIV1_OUTDIV2(1)|SIM_CLKDIV1_OUTDIV3(1)|SIM_CLKDIV1_OUTDIV4(3));
  MCG->C5 = (uint8_t)0x00u;
  MCG->C6 = (uint8_t)0x00u;
